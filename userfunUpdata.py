...
def home():
    ...
    try:
        ...
        routerClients = api.get_resource('/ppp/active').get()
        numberOfClient = len(routerClients)

        clientNames = [routerData['name'] for routerData in routerClients]
        onlineClient = collections.count_documents(
            {"name": {"$in": clientNames}})
        ...
        db_total_client = collections.count_documents({"nodetype": "user"})
        onlineClient = 0
    ...
    return render_template('index.html', message=message, total=db_total_client, online=onlineClient,
...
def tree():
    ...
    for updatedata in dataUpdateFind:
        collections.update_one(updatedata, {"$set": offline}, upsert=True)

    if routerDatas:
        for routerData in routerDatas:
            routerClientData = routerData["name"]
            dbDatas1 = collections.find(
                {"name": routerClientData}, projection={"rootId": 1})
